...
For each headline's cited URL that resolves successfully, the verifier SHALL verify that the article's published date matches `today_date`.
...
- **GIVEN** `today_date = "2026-06-08"`
- **GIVEN** `extract_url` returns content with `Published: 2025-12-31`
- **THEN** verifier calls `web_search` with the headline text and today's date to find a replacement
