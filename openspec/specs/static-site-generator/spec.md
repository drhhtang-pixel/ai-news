...
`publish.py` generates a static GitHub Pages site from `summaries.md`, producing per-day HTML pages, a listing index, and a bilingual about page under the `docs/` directory.

## Overview

- **Script**: `publish.py`
- **Input**: `summaries.md` — append-only file of `## YYYY-MM-DD HH:MM` sections
- **Output**: `docs/` tree — one subdirectory per date + `docs/index.html` + `docs/about.html`
- **Idempotent**: all daily pages are fully regenerated on every run
